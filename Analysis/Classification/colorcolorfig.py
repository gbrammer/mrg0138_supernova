...
"""
kfo 02.12.20
Color color fig updated slightly
1. Gave extra optional arguments for sncosmoSim objects relevant for salt. 
    * included c_range, x1_range, x0_range. (Previously only arguments were z_range,t0_range). They will default to arguments already provided if not explicitly provided.
2. Made a function _plot_colormag_singlesimplot()
    *  x ~ (band1 - band2) y ~ (band3); exactly analogous to the already available colorcolor there
3. The color mag is implemented in the three plotting functions (contours, points, gradient) with an added argument colorcolor
    * Colorcolor will default to true and do the colorcolor initially designed, but just provide the arg as False to get color mag now instead 
4. Some other small py2->py3 changes 
    * cPickle ~ _pickle, dict.keys() ~ list(dict.keys()) for iterating etc (these are all included w #kfo comment nearby)  
"""
...
class SncosmoSim( object ):
    ...
    def __init__(self, sntype, observations=None, z_range=[1.8,2.2],
                 t0_range=[0,0], c_range=[0,0.1],x1_range=[0,1.], nsim=100, perfect=True,
        ...
        if mwdust:
            ...
            print('kfo hmm dont yet got mwdust')
        ...
        for isim in range(self.nsim):
            ...
            if subclass =='Ia' :
                ...
                # x1 = normal(0., 1.)
                # c = normal(0., 0.1) 
                x1 = normal(x1_range[0],x1_range[1])
                c = normal(c_range[0],c_range[1])
...
def sncosmo_sim( snroot='nebra', z_range=[1.4,2.3], t0_range=[-20,20],c_range=[0,0.1],x1_range=[0,1.],
                 filterset='hst',nsim=1000, verbose=True, clobber=False,mwdust=False,mwEBV=.018):
    ...
    if os.path.isfile( simIapkl ) and not clobber>1 :
        ...
        simIa = SncosmoSim( 'Ia' , z_range=z_range, t0_range=t0_range,c_range=c_range,x1_range=x1_range,nsim=nsim, filterset=filterset,mwdust=mwdust,mwEBV=mwEBV)
...
def _plot_colormag_singlesim(snsim, band1, band2, band3,
                               plotstyle='points',
                               nbins=None, colorcolor=True,contour_range=None,**plotargs):
...
def plotcontours( sim1, sim2, sim3=None, band1='f350lp', band2='f125w',
                  band3='f160w', nbins=None,colorcolor=True,contour_range=None, **plotargs ):
    ...
    if colorcolor:
        ...
        ax = _plot_colormag_singlesim(sim1, band1, band2, band3, nbins=nbins, plotstyle='contourf',contour_range=contour_range, **plotargs1)
        ax = _plot_colormag_singlesim(sim2, band1, band2, band3, nbins=nbins, plotstyle='contourf', contour_range=contour_range,**plotargs2)
        if sim3 is not None :
            ax = _plot_colormag_singlesim(sim3, band1, band2, band3, nbins=nbins, plotstyle='contourf', contour_range=contour_range,**plotargs3)
        return( ax )
...
def colorcolorplot(sim1=None, sim2=None, sim3=None,
    ...
    plotnebra(band1=band1,band2=band2,band3=band3)
    if sim3 is not None :
